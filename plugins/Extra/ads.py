...
async def set_ads(client, message):
    try:
        ...
        if '#' not in command_args or len(command_args.split('#')) < 3:
            await message.reply_text(f"Usage: /set_ads {{ads name}}#{{time}}#{{photo URL}} <a href=https://t.me/MrAK_developer/9>Explain</a>")
            return
        ...
        await db.MrAK_set_ads_link(url)
...
async def del_ads(client, message):
    try:
        ...
        current_link = await db.MrAK_get_ads_link()
        if current_link:
            is_deleted = await db.MrAK_del_ads_link()
            if is_deleted:
