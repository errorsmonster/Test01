...
class Database:
    ...
    def __init__(self):
        ...
        self.MrAK_ads_link = mydb.MrAK_ads_link
    ...
    async def give_free_trial(self, user_id):
     ...
     # MrAK BOTS = @IM_MrAK
    async def MrAK_set_ads_link(self,link):
        await self.MrAK_ads_link.update_one({} , {'$set': {'link': link}} , upsert=True)
    async def MrAK_get_ads_link(self):
        link = await self.MrAK_ads_link.find_one({})
        if link is not None:
            return link.get("link")
        else:
            return None
            
    async def MrAK_del_ads_link(self):
        try: 
            isDeleted = await self.MrAK_ads_link.delete_one({})
